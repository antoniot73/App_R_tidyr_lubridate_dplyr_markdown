...
#
# Propósito:
#   Generar dashboard_911/app.R a partir de una plantilla Shiny
#   validada para el proyecto 911 CDMX.
#
# Ajuste incluido:
#   En la pestaña "Inicio" se agregan/garantizan los segmentos:
#   1. Preparación de insumos para despliegue en GitHub Pages.
#   2. Referencias bibliográficas.
#
#   El bloque de preparación debe mostrar explícitamente:
...
#   Puede ejecutarse desde la raíz del proyecto o desde dashboard_911/.
#
# Uso recomendado:
#   source("crear_app_shiny_911.R")
#
# Variables opcionales:
#   CREAR_APP_SHINY_911_ROOT       Ruta raíz o carpeta dashboard_911.
#   CREAR_APP_SHINY_911_AUTO       TRUE/FALSE. Ejecutar automáticamente.
#   CREAR_APP_SHINY_911_OVERWRITE  TRUE/FALSE. Sobrescribir app.R. Por defecto TRUE.
# ============================================================
...
log_crear_app_911 <- function(nivel, mensaje) {
...
#'
#' @param valor character. Texto a convertir.
#' @param default logical. Valor por defecto.
#' @return logical. Valor convertido.
as_logical_crear_app_911 <- function(valor, default = FALSE) {
  tryCatch(
    {
    ...
    error = function(e) {
      log_crear_app_911("WARN", paste("No se pudo convertir valor lógico:", e$message))
      default
    }
  )
}
...
detectar_script_path_crear_app_911 <- function() {
  tryCatch(
    {
      frames <- sys.frames()
    ...
    error = function(e) {
      log_crear_app_911("WARN", paste("No se pudo detectar ruta del script:", e$message))
      ""
    }
  )
}

#' Inferir ruta base portable
#'
#' Prioridad:
#'   1. Variable CREAR_APP_SHINY_911_ROOT.
#'   2. Carpeta donde está este script.
#'   3. getwd() como respaldo.
#'
#' @return character. Ruta base normalizada.
inferir_base_crear_app_911 <- function() {
  tryCatch(
    {
      root_env <- Sys.getenv("CREAR_APP_SHINY_911_ROOT", unset = "")

      if (nzchar(root_env)) {
        base_path <- normalizePath(root_env, winslash = "/", mustWork = FALSE)
        log_crear_app_911("INFO", paste("Ruta base tomada desde CREAR_APP_SHINY_911_ROOT:", base_path))
        return(base_path)
      }

      script_path <- detectar_script_path_crear_app_911()

      if (nzchar(script_path)) {
        base_path <- normalizePath(dirname(script_path), winslash = "/", mustWork = FALSE)
        log_crear_app_911("INFO", paste("Ruta base inferida desde ubicación del script:", base_path))
        return(base_path)
      }

      base_path <- normalizePath(getwd(), winslash = "/", mustWork = FALSE)
      log_crear_app_911("INFO", paste("Ruta base inferida desde getwd():", base_path))
      base_path
    },
    error = function(e) {
      log_crear_app_911("ERROR", paste("No se pudo inferir ruta base:", e$message))
      stop(e$message)
    }
  )
}

#' Resolver rutas del proyecto y de Shiny
#'
#' @param root character | NULL. Ruta raíz del proyecto o carpeta dashboard_911.
#' @return list. Rutas principales.
resolver_rutas_crear_app_911 <- function(root = NULL) {
  tryCatch(
    {
      if (missing(root) || is.null(root) || !nzchar(root)) {
        root <- inferir_base_crear_app_911()
      }

      root <- normalizePath(root, winslash = "/", mustWork = FALSE)

      if (identical(basename(root), "dashboard_911")) {
      ...
      rutas <- list(
        ...
        data_dir = normalizePath(file.path(app_dir, "data"), winslash = "/", mustWork = FALSE),
        www_dir = normalizePath(file.path(app_dir, "www"), winslash = "/", mustWork = FALSE),
        app_r = normalizePath(file.path(app_dir, "app.R"), winslash = "/", mustWork = FALSE)
      )

      log_crear_app_911("INFO", paste("Raíz del proyecto:", rutas$project_root))
      log_crear_app_911("INFO", paste("Carpeta Shiny:", rutas$app_dir))
      log_crear_app_911("INFO", paste("app.R destino:", rutas$app_r))

      rutas
    },
    error = function(e) {
      log_crear_app_911("ERROR", paste("No se pudieron resolver rutas:", e$message))
      stop(e$message)
    }
  )
}
...
#' @return NULL invisible.
crear_directorios_crear_app_911 <- function(rutas) {
  tryCatch(
    {
      requeridos <- c(rutas$app_dir, rutas$data_dir, rutas$www_dir)

      for (ruta in requeridos) {
        if (!dir.exists(ruta)) {
          dir.create(ruta, recursive = TRUE, showWarnings = FALSE)
          log_crear_app_911("OK", paste("Directorio creado:", ruta))
        } else {
          log_crear_app_911("INFO", paste("Directorio existente:", ruta))
        }
      }
    ...
    error = function(e) {
      log_crear_app_911("ERROR", paste("Error creando directorios:", e$message))
      stop(e$message)
    }
  )
}

#' Crear respaldo de app.R existente
#'
#' @param ruta_app character. Ruta de app.R.
#' @return character | NULL. Ruta de respaldo o NULL.
crear_backup_app_existente_911 <- function(ruta_app) {
  tryCatch(
    {
      if (!file.exists(ruta_app)) {
        return(invisible(NULL))
      }

      timestamp <- format(Sys.time(), "%Y%m%d_%H%M%S")
      backup <- paste0(ruta_app, ".bak_", timestamp)
      ok <- file.copy(ruta_app, backup, overwrite = FALSE)

      if (!isTRUE(ok)) {
        stop("No se pudo crear backup de app.R existente.")
      }

      log_crear_app_911("OK", paste("Backup creado:", backup))
      backup
    },
    error = function(e) {
      log_crear_app_911("ERROR", paste("Error creando backup:", e$message))
      stop(e$message)
    }
  )
}

#' Construir plantilla completa de app.R
#'
#' @return character. Líneas del archivo app.R.
construir_app_r_911 <- function() {
  tryCatch(
    {
      c(
      "# ============================================================",
    ...
    error = function(e) {
      log_crear_app_911("ERROR", paste("Error construyendo plantilla app.R:", e$message))
      stop(e$message)
    }
  )
}

#' Escribir app.R en dashboard_911
#'
#' @param rutas list. Rutas principales.
#' @param overwrite logical. Si TRUE sobrescribe app.R existente.
#' @return character. Ruta de app.R generado.
escribir_app_r_911 <- function(rutas, overwrite = TRUE) {
  tryCatch(
    {
      if (file.exists(rutas$app_r) && !isTRUE(overwrite)) {
        log_crear_app_911("WARN", paste("Ya existe app.R y overwrite=FALSE. No se sobrescribió:", rutas$app_r))
        return(rutas$app_r)
      }

      if (file.exists(rutas$app_r) && isTRUE(overwrite)) {
        crear_backup_app_existente_911(rutas$app_r)
      }

      contenido <- construir_app_r_911()
      writeLines(contenido, rutas$app_r, useBytes = TRUE)

      if (!file.exists(rutas$app_r)) {
        stop("No se pudo crear app.R.")
      }

      log_crear_app_911("OK", paste("app.R generado:", rutas$app_r))
      rutas$app_r
    },
    error = function(e) {
      log_crear_app_911("ERROR", paste("Error escribiendo app.R:", e$message))
      stop(e$message)
    }
  )
}

#' Validar app.R generado
#'
#' @param ruta_app character. Ruta de app.R.
#' @return NULL invisible.
validar_app_generada_911 <- function(ruta_app) {
  tryCatch(
    {
      parse(ruta_app)

      lineas <- readLines(ruta_app, warn = FALSE, encoding = "UTF-8")

      requeridos <- c(
        "crear_bloque_preparacion_github_pages <- function",
        "crear_bloque_preparacion_github_pages()",
        "Preparación de insumos para despliegue en GitHub Pages",
        "source(\\\"render_rmarkdown_github_pages_911.R\\\")",
        "source(\\\"crear_app_shiny_911.R\\\")",
        "source(\\\"probar_shiny_local_911.R\\\")",
        "crear_bloque_referencias_bibliograficas_inicio <- function",
        ...
        "C5 de la Ciudad de México",
        "R Markdown: The definitive guide",
        "crear_ui <- function",
        "crear_server <- function",
        "main <- function"
      )

      for (patron in requeridos) {
        if (!any(grepl(patron, lineas, fixed = TRUE))) {
          stop("Falta patrón requerido en app.R generado: ", patron)
        }
      }

      log_crear_app_911("OK", "Validación sintáctica y estructural de app.R correcta.")
      invisible(NULL)
    },
    error = function(e) {
      log_crear_app_911("ERROR", paste("Validación de app.R fallida:", e$message))
      stop(e$message)
    }
  )
}

#' Crear app.R de Shiny 911
#'
#' @param root character | NULL. Ruta raíz del proyecto o carpeta dashboard_911.
#' @param overwrite logical. Si TRUE sobrescribe app.R existente.
#' @return character. Ruta de app.R generado.
crear_app_shiny_911 <- function(root = NULL, overwrite = TRUE) {
  tryCatch(
    {
      rutas <- resolver_rutas_crear_app_911(root)
      crear_directorios_crear_app_911(rutas)

      ruta_app <- escribir_app_r_911(
        rutas = rutas,
        overwrite = overwrite
      )

      validar_app_generada_911(ruta_app)

      cat("\n============================================================\n")
      cat("PIPELINE crear_app_shiny_911 TERMINADO\n")
      cat("============================================================\n")
      cat("app.R generado:\n")
      cat(ruta_app, "\n\n")
      cat("Segmentos garantizados en Inicio:\n")
      cat("Preparación de insumos para despliegue en GitHub Pages\n")
      cat("Referencias bibliográficas\n\n")
      cat("Siguiente paso recomendado:\n")
      cat("source('probar_shiny_local_911.R')\n")
      cat("============================================================\n\n")

      ruta_app
    },
    error = function(e) {
      log_crear_app_911("ERROR", paste("No se pudo crear app.R:", e$message))
      stop(e$message)
    }
  )
}
...
#'
#' @return character | NULL. Ruta de app.R generado si se ejecuta.
ejecutar_auto_crear_app_shiny_911 <- function() {
  tryCatch(
    {
      auto <- as_logical_crear_app_911(
        Sys.getenv("CREAR_APP_SHINY_911_AUTO", unset = "TRUE"),
        default = TRUE
      )

      if (!isTRUE(auto)) {
        log_crear_app_911("INFO", "Ejecución automática desactivada.")
        return(invisible(NULL))
      }

      overwrite <- as_logical_crear_app_911(
        Sys.getenv("CREAR_APP_SHINY_911_OVERWRITE", unset = "TRUE"),
        default = TRUE
      )
    ...
    error = function(e) {
      log_crear_app_911("ERROR", paste("Error en ejecución automática:", e$message))
      stop(e$message)
    }
  )
}

ejecutar_auto_crear_app_shiny_911()
